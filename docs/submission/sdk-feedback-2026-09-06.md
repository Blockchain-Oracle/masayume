...
Our correction uses the feed's declared decimals, rejects invalid or irreconcilable inputs, and leaves the provider's original oracle units unchanged for settlement consumers. Tests cover 6-, 8- and 18-decimal feeds, both directions, sub-threshold holds and correct prompt prices. Sensei exposed a related local formatting mistake: a decimal-count constant `2` was used as a divisor instead of `10^2`; its deployed fix converts oracle cents to whole dollars deliberately. The corrected rehearsal later produced a genuine AI fill and losing settlement, and a corrected Momentum fill and winning settlement; exact receipts and evidence limits remain in the acceptance ledger.
