...
| 2026-09-03 / claude | Two assets, one Window per cadence, measured live | A deck may **mix cadences** when neither 15m nor 1h has three eligible Windows | A deck's cards can run for different lengths, each with its own countdown; both players hold identical cards either way | **Deviation from §Owner decisions 4 — owner review** |
| 2026-09-03 / claude | 76s before a boundary: 5m, 15m and 1h all locked together | Near a boundary no deck is dealable at all, so a paired match is **held for three minutes** rather than dissolved | Two matched players are told "waiting for the next Windows to open" instead of being returned to the queue with an error | Implementation, forced by the venue |
