...
| 2026-09-03 / claude | Owner's 2026-09-01 reel-card ruling; RESUME §Never write `var(--white)` | The `/games` frame follows the theme rather than being a dark stage | Games read correctly in light mode; the arcade canvas in slice 4 stays a dark island because its ground is part of the mechanic | Follows an existing owner decision |
| 2026-09-03 / claude | `game_settings.reduced_motion` is nullable; the schema's signature-check note | Stage settings are device-local; the `game_settings` sync waits for the games to have a session | Sound, haptics, motion and accent work signed out and apply immediately; nothing is signed to change a toggle | Deferred, not dropped |
| 2026-09-03 / claude | Tailwind v4 minted `.pl-section` from `--spacing-section` | `pl-`-prefixed component classes must not collide with Tailwind's padding-left namespace; renamed to `.pl-block` | `/parlay` and `/games/range` lose a phantom 64px indent that cost a phone a sixth of its width | Defect fix |
| 2026-09-03 / claude | `GameArena.fork.t.sol` on Shannon; the live smoke at `0xec71…f0dF` | The arena's challenger is NAMED at `createMatch`, not left open | A stranger cannot join a match whose deck commitment already binds the real opponent's client seed | Implementation, forced by the commitment |
| 2026-09-03 / claude | `ArenaCommitment`; the golden vector in two suites | `revealDeck` is permissionless — the commitment, not a key, proves the deck was fixed first | No `DECKMASTER_PRIVATE_KEY` and no liveness dependency on one signer to open a deck | Implementation, removes a dependency |
| 2026-09-03 / claude | `lifecycle.ts` "a forfeited match still settles its cards" | Card settlement is independent of the pot: `settleCard` works in every status past the reveal | Losing a pot never costs a player the market position they paid for | Implementation, follows core |
| 2026-09-03 / claude | Two cards on one Window settle on one print | A deck may not carry the same Window twice, and `minCardLifeSec` must outlast `pickWindowSec` | A deck measures three to five independent calls, and the last swipe of a legal deck can never revert on a dead Window | Implementation guard |
| 2026-09-03 / claude | Live `createMatch` 1,104,046 vs the fork's 185,484 | Fork gas is not live gas; the `arena` envelope is anchored on PrivateDesk's live mint (1,917,880) at 8M | An under-provisioned pick would forfeit a card and its side-pot; the envelope costs a player 0.048 STT instead | Correction to the deploy proof |
| 2026-09-03 / owner | `spike:deck-supply`: 40.1% dealable at `minDeckSize` 3 with 180/120 windows | **`minDeckSize` 3→2, join 180→60s, reveal 120→45s**, written on chain (`0x261fe1cc…2b13`) | The duel is available 89% of the time instead of 40%; a deck is 4 cards normally and 2 in the dead zone | **Approved** — supersedes §Owner decisions 4's three-card floor |
| 2026-09-03 / claude | Two assets, one Window per cadence, measured live | A deck takes **every eligible Window** soonest-first rather than preferring one cadence; `lane` is descriptive | A four-card deck whenever the venue has four; its pot waits on the slowest card, bounded by the one-hour horizon | Follows the owner's approval above |
| 2026-09-03 / claude | The arena checks card life at REVEAL, not at deal | Headroom at deal is `minCardLifeSec + joinWindow + revealWindow + create latency`, not `minCardLifeSec + 60` | A slow challenger no longer strands a deck that cannot be opened and must refund | Correction — a latent bug found by reading the deployed parameters |
| 2026-09-03 / claude | 76s before a boundary: 5m, 15m and 1h all locked together | Near a boundary no deck is dealable at all, so a paired match is **held for three minutes** with a countdown rather than dissolved | Two matched players are told when the next Windows open, instead of being returned to the queue with an error | Implementation, forced by the venue |
| 2026-09-03 / claude | The first `queue.update` reported `nextDeckInSec: null` while the first supply read was still in flight | Absent means "not known yet"; `null` means "further out than the projection looked". They are different answers and a screen must not conflate them | Nobody is shown "no deck for the foreseeable future" for a deck that is one tick away | Implementation |
| 2026-09-03 / claude | One card emits `PickFilled` and later `CardSettled` | A receipt is keyed by `chainId:matchId:cardIndex:seat`, never by a chain log identity | A settled card fills in its payout instead of appearing twice, and a deck cannot lock one pick short | Correction to slice 1's types |
| 2026-09-03 / claude | `IGameArena.RefundReason.CREATOR_CANCELLED` | Core's first refund reason is `creator-cancelled`, not `creator-timeout` | A creator who withdraws an unjoined match is not told a clock ran out | Correction, contract is the authority |
| 2026-09-03 / claude | A WebSocket upgrade is a GET with no body | The room token travels in `Sec-WebSocket-Protocol`; a refused upgrade is an HTTP 401 | A credential never lands in an access log or a `Referer`, and a client can tell an expired token from a dead server | Implementation |
| 2026-09-03 / claude | A second table in one Postgres shares one outage | The sealed deck reveal is **journalled to disk first**, and the `duel_decks` row is the queryable copy | A deployment with no database can still deal a deck and still open it; losing the journal is what refunds a match | Implementation of §"a second recoverable copy" |
| 2026-09-03 / claude | The queue drive deadlocked, then never re-paired | `match.found` is sent **at pairing**, and pairing is attempted **on every tick**, not only on `queue.join` | The ceremony can start at all, and the widening Elo band actually pairs players it widened to reach | Two defects the live drive found |
| 2026-09-03 / claude | `settleCard` reverts cost a book walk's gas on Somnia | The settler restates every crank's precondition in a tested pure function before it sends | A settler that guesses wrong is a slow leak of the operator's STT | Implementation |
