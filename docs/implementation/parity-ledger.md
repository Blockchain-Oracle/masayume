...
| 2026-09-03 | **Shannon's cadences are aligned, and that changed the deck policy twice.** The venue runs two assets with one Window per cadence, so there are at most TWO live 15m Windows at any moment — `selectDeck`'s approved "three to five distinct Windows of one cadence" could never deal a deck, and refusing would have made the duel undeliverable, so a **mixed lane** was added below the two single-cadence ones. Measured again 76 seconds before a boundary: the 5m, 15m and 1h Windows were **all locked at once**, leaving only the 4h/1d pair, which is outside a duel's one-hour horizon — so for the last minutes of a cycle no deck is dealable at all. The matchmaker now holds a paired match for three minutes and tells both players it is waiting for the next Windows, instead of throwing them out of the queue | `spike:arena-open` candidate table; `deck.ts` §The mixed lane; doc 06 §Decision log | A duel's cards may differ in cadence, each with its own countdown; a queue can say "waiting for the next Windows" for a couple of minutes near a boundary | Deviation from doc 06 §Owner decisions 4 — **needs owner review**, forced by the venue's supply |
