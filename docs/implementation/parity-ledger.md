...
flicky_commit: 56054ba (reference/flicky — the duel's lifecycle, sound, motion, flow and design)
pips_commit: fe8f696 (reference/pips — the hub, press physics, control audio, arcade)
games_authority: inside /games, Pips and Flicky govern sound, motion, flow and visual language in Yosuku's tokens (owner, 2026-09-04); Yosuku remains the shell's authority
review_table: docs/implementation/needs-user-review-2026-09-04.md
---
...
|---|---|---|---|---|
| 2026-09-05 | **The ledger plate follows the theme.** The reference draws `.ledger-plate` (and the `.plate-rows` frame under it) as a cream slab with fixed inks in both themes; ours keeps that exactly in light and turns to the theme's surface and inks in dark. `ledger-plate.css` owns the five `--lp-*` tokens; the pool rows, the X-Predict wallet card (`x-card.css`) and the docs disclosure read them. Commit `d1cfc5f`; context/58 §8 | `components/portfolio/BalancePlate.tsx`, `XWalletCard.tsx`, part-07 `.ledger-plate` | In dark mode the portfolio plate and the X card are dark with white ink instead of a cream slab | Approved (owner, 2026-09-05: "on dark mode this is still showing as light mode… the X Predicts Wallet… hard to see") |
| 2026-09-05 | **The agent persona is drawn, not fetched — and it moves.** The reference renders each agent as a DiceBear notionists face from `api.dicebear.com` (seed, `backgroundColor=f4eee1`, `radius=12`); our CSP allows no external image host, so the mark had been a seeded letter. `AgentPortrait` now draws the same persona locally with `@dicebear/core` + `@dicebear/notionists` (MIT) and adds an idle breath and a hover nod (none under reduced motion). The letter glyph and its two other tiles are deleted. Commit `d1cfc5f` | `lib/sui/strategyClient.ts` (persona URL), `components/strategies/*` | Every agent shows the reference's face everywhere it appears; the leaderboard's letters are its own and unchanged | Approved (owner, 2026-09-05: "their own animations or characters… instead of just writing G or H") |
| 2026-09-05 | **The strategies desk reads the theme tokens.** Its screens and stylesheets were dark-mode literals (`text-white/40`, `rgba(255,255,255,…)`, panels on the fixed `--color-bg`); light mode showed pale text on cream and black slabs. Every literal now reads the ink ladder, `color-mix` on `--white`, the `--gray-*` ladder and `--bg`. Commit `d1cfc5f` | `components/strategies/*`, `app/agents/*` | Light mode on `/strategies`, `/agents`, the studio and the copy drawer reads as cream and ink, as the reference does | Fix (owner, 2026-09-05) |
| 2026-09-05 | **The X rail runs on the account's own session, not X's API.** X's deprecated Free plan serves no v2 endpoint and pay-per-use wants credits the owner will not buy for a test; the relay reads mentions and replies through `rettiwt-api` with the account's session cookies as its key (`9d7ea1b`, `87c19f8`), and Sign in with X is the three-legged OAuth 1.0a flow the plan still serves, whose last step returns the identity itself (`7283967`). Proven end to end on the live site; then the X API v2 transport and the OAuth 2.0 client were deleted (`9b077b1`) under the owner's no-legacy rule. The reference's mechanics — signed session, wallet link, EXECUTOR grant, one receipt per mention, a reply that never states a number that was not booked — are unchanged | context/58 §7; rettiwt `/rishikant181/rettiwt-api` | Tweet `@masayume_app <btc\|eth> <up\|down> <stake> <cadence>` and the agent places it from your own funds; the reply says exactly what happened | **Owner, 2026-09-05** ("use the library… without having to pay"; "clean up the old implementation") |
| 2026-09-04 | **Lucky, the arcade pair, Moonshot A and the AI-agent preset are built** — four builders in worktrees, merged `fb426e1` (Moonshot), `2b89fd6` (arcade), `1e3f189` (agent), `e863a42` (Lucky); the hub lists all seven modes live. Product rulings taken with them: Moonshot's rung caps (25× ≤ 100, 10× ≤ 200 tUSDC payout) over the contract's 500; an arcade run longer than the replay budget is refused, never accepted "envelope-checked"; the calm ramp is a tagged option on the same board; Lucky's void is neither win nor loss for streaks and a cashed-out spin is unverified; no model string in an agent's on-chain spec (recorded per decision) | context/58 §5; the planners' files | Seven playable modes; a creator can launch an agent | Built on the owner's approvals of 2026-09-04 (Moonshot A; the order Lucky → arcade → Moonshot; the AI agent from the review) |
| 2026-09-04 | **The maker ran out of gas** — 0.558 STT under its 0.576 write envelope, refusing every quote and settle since the RPC outage; +6 STT from the deployer. The venue is degraded after the outage (no 5m/15m Windows, 1h prints pending), which blocks the live spins | context/58 §6 | Books were thin because of us; they refill when the venue reopens the short cadences | Operational — recorded |
| 2026-09-04 | **The 45 deviations are approved.** Every row that read `Needs user review` (the table in `needs-user-review-2026-09-04.md`) is kept as built; the markers now read `Approved (owner, 2026-09-04)`. Any row may still be named later to revert | `needs-user-review-2026-09-04.md` | Nothing changes on screen; the ledger stops carrying open questions | **Owner, 2026-09-04** ("I actually approve of everything") |
| 2026-09-04 | **One money rail.** The Add-money modal's "Buy with a card" and "Deposit from another chain" rows and the `/fund` page (the reference's Paystack on-ramp, disabled here with its reason) are removed: the testnet has the venue's faucet mint and nothing else, and a row that leads nowhere is the dead end the reference's own header comment refused. The account menu's dead `/claims` link went with them | `AddFunds.tsx`; `funding/copy.ts`; `nav-items.ts` | The modal is title, body, account, one mint button, the STT-faucet foot | **Owner, 2026-09-04** ("there should be nothing like buy from card") |
| 2026-09-04 | **Six routes removed** — `/social` (an internal marketing copy board in the reference, noindex), `/waitlist` (a pre-launch founder waitlist), `/creators` (the X creator guide — its material goes into our documentation later), `/creator/studio` and `/creator/recover` (a card studio earning a builder fee Event Contracts do not have), `/studio` (a passphrase-gated founder posting tool). Build loses its Create section; Explore's Community folds into Proof | reference `app/social/SocialBoard.tsx`, `app/waitlist/page.tsx`, `app/creators/page.tsx`, `components/CreatorCardStudio.tsx`, `app/studio/page.tsx`; context/05 §9 | Six `CapabilityPending` plates and their nav entries are gone; the drawer holds 26 destinations | **Owner, 2026-09-04** (the exception to "no drop list", decided on what the reference contains) |
| 2026-09-04 | **The portfolio's Add money opens the modal in place** — the reference's primary button routes to `/markets` | `PortfolioScreen.tsx` L89 → `openFunds()` | A button that says Add money adds money | **Owner, 2026-09-04** (asked twice) |
| 2026-09-04 | **The Ticket's leverage chips say "Leverage"** beside them, in the Ticket's control-label type; an enabled boost carries a one-line title. The reference keeps the word in `aria-label` only | `LeverageChips.tsx`; reference `Ticket624Drawer.tsx` L1074–1090 | Nobody reading 1×/2×/3× has to guess what they are | **Owner, 2026-09-04** ("people will not know that it's leverage") |
| 2026-09-04 | **The Room remembers its join for the token's hour.** The join token is kept per wallet and market (memory + `localStorage`, as the reference keeps its room identity) and read back when the Room mounts; it used to die with the sheet's subtree, so every reopen re-asked the wallet | `room-session.ts`; `useRoom.ts`; reference `roomDelegate.ts` L30–37 | One signature an hour, not one per opening | **Owner, 2026-09-04** ("I signed once") |
| 2026-09-04 | **Game settings are Flicky's menu plate** — the centred `.du-modal` the HOW TO already uses, the two volume sliders in one well, capped at 80dvh — not a bottom sheet with the viewport as its ceiling | Flicky `menu-modal.tsx` L56–113; `GameSettingsSheet.tsx` | The settings no longer open as a full page on a phone | **Owner, 2026-09-04** |
| 2026-09-04 | **The collateral's mark** — a USDC-style disc drawn as paths, on the money pill (for lucide's `Coins`) and the mint button. No reference precedent; no token art existed | `AssetMarks.tsx` `TUsdcMark`; `icons.css` | The money surfaces show the token's logo | **Owner, 2026-09-04** ("use the tUSDC logo") |
| 2026-09-04 | **Deal-headroom rule D** — `dealHeadroomSec` counts the pick window, so the arena's card-life floor holds for every second a player may swipe; `DECK_POLICY_VERSION` 3 → 4 | `deckmaster.ts` L141; context/54 §5 | A slow player's last swipes are never refused as `TooLate`; decks are dealable ~84% of the time instead of ~89% (to be re-measured) | **Owner, 2026-09-04** |
| 2026-09-04 | **Doc 04 amended (8e)** — line 20 now records Pips/Flicky as the authority for the games' sound, motion, flow and visual language inside `/games`, in Yosuku's tokens with the arts redrawn; §Pick execution records what shipped: the reference's sponsored atomic tx per card against our match-scoped key named and funded at entry | `04-game-system.md` L20, §Pick execution | — | Recorded |
| 2026-09-04 | **The home tile's live PnL sparkline waits on per-card marks** — the projector holds fills and settlements, not a price mark per card | `my-match-tile.tsx` `StreamingPnlChart` | The tile shows opponent, cards settled and live, without the chart | Pending — needs the projector to mark each open card against the feed |
| 2026-09-04 | **Pips's first-run onboarding (handle → accent → welcome) and the achievement celebration wait on their stores** — the accent exists; there is no handle store and achievements have no evidence writer yet | Pips `components/onboarding/*`, `achievements/*` | The hub still opens on the selection; the achievements plate names its dependency | Pending — sequenced behind the arcade score API and a profile store, as doc 04 §Practice and arcade state already says |
| 2026-09-04 | **Flicky's Season 0 prize overlay is not carried** — no season programme, no funded pool; a prize breakdown over nothing would be a fabrication. The ladder is rating-only and says so | `rank.tsx` (season banner, `prizeForRank`, eligibility) | The ladder has no prizes column | **Blocked** — needs a season programme with a funded pool; recorded, not dropped |
| 2026-09-04 | **The duel's flow is Flicky's** — `/games/duel/[matchId]` is shareable and reload-safe (a seat resumes through the room's `resumeMatchId`; anyone else gets the read-only result straight off the arena, with the share); `/games/history` (every duel, newest first, verdict, PnL, when; 8 s poll) and `/games/rank` (the ladder over the settler's ratings, the wallet's own row beneath the cut; 10 s poll) replace two `PendingPlate`s; the hub's `MatchTile` (opponent, cards settled, live, open) and Pips's remembered last game; a per-game HOW TO overlay from the rail; the rail hides while swiping; dead-duel detection with the arena's own `lockPicks` crank and the opponent's presence said plainly; the pending pages read the catalog's dependency (they disagreed). Arts: the bull, bear, coin, card back, searching banner and locked-in mark drawn as our own pixel grids in the venue's tokens; the CRT face on the art window; m6x11plus (credited) on the frame's display bits | Flicky `routes/game/play.tsx`, `duel-view.tsx` L383–428 and L576–580, `my-match-tile.tsx` L277–330, `history.tsx`, `rank.tsx`, `layout.tsx` L91–93; Pips `lib/storage.ts` (last game), the HOW TO overlay | A match has a link; a finished one can be read by anyone; history and the ladder exist; the deck peeks its back, the mascot reacts to the lean | User decision 2026-09-04 (Pips/Flicky govern the games' flow and design, in Yosuku's tokens; arts recreated) |
| 2026-09-04 | **The games move as Flicky and Pips move** — `motion.ts` (Pips's presets, Flicky's two card timings) as the one vocabulary; the drag carries five signals from one motion value (18° tilt, the side's tint rising to the commit line, an UP/DOWN stamp past 24px, `data-swipe` for the face, the next card rising and growing with the finger) and the throw leaves opaque and spinning; every clock ramps profit → vermilion (≤10 min) → loss (≤2 min) and pulses in the last 30 s; a full-width depletion bar drains the pick window a second at a time; the queue arrives in 280 ms with a breathing plate, bouncing dots and a real leave button; each settled card flips to its side's tint and the live one carries a pulsing dot; Flicky's result modal (verdict 4xl tracked, return 5xl, two 64px seats, three figures, `share image` through the app's own card flow, `copy link`) opens itself with the verdict's sound; Pips's press physics on every control; the checker canvas drifting 12 s under `/games` in Yosuku's surfaces; a 260 ms route entrance. Reduced motion short-circuits all of it through the frame's one attribute | Flicky `swipe-screen.tsx` L159–297, `duel-result-modal.tsx` L148–231, `pvp.tsx` L357–430, `duel-view.tsx` L814–826, `globals.css` L49–78 and L748–766, `active-duel.tsx` L399–418; Pips `utils/motion.ts`, `styles.css` L199–211 | The duel feels like the reference; the CRT finish and the mascot swap wait on the redrawn art (8d) | User decision 2026-09-04 (Pips/Flicky govern the games' motion and design, in Yosuku's tokens) |
| 2026-09-04 | **The games have Flicky's sound and Pips's press** — ten Kenney CC0 effects (`public/sounds/SOURCES.md`, provenance to the packs), `audio.ts` re-implemented in our idiom (gesture unlock, decoded buffers, every error swallowed, two persisted sliders), Flicky's eight cue moments wired (swipe-up/down at commit, match-found before the swap, card-win/loss only on a settlement seen live, duel-win/lose/tie as the verdict lands, modal open/close, click on every control), Pips's press/release discipline on every control under `/games` (a varied press, its release in the same variation, a 120 ms floor), the practice loss gets its own sound. No music bed: Flicky's is Uppbeat-licensed — Blocked on a CC0 track; the slider hides until one exists | Flicky `lib/sound.ts`, `pixel-button.tsx` L45, `duel-view.tsx` L237–249, `duel-result-modal.tsx` L70–78, `pvp.tsx` L89, `layout.tsx` L102–107; Pips `consoleAudio.ts` L110–157 | The duel sounds like the reference; the settings sheet has two sliders instead of a switch | User decision 2026-09-04 (Pips/Flicky govern the games' sound); assets CC0, code ours |
| 2026-09-04 | **The duel signs once — the entry — and a match-scoped key swipes** — `GameArena` split along its seam into `ArenaMatches` (ledger, refunds) + `ArenaAgents` (a per-seat `Agent {agent, expiresAtSec, budgetBase, spentBase}`, `authorizeAgent`, `agentOf`) + `GameArena` (trading); `createMatchWithAgent`/`joinMatchWithAgent` are `payable` and forward their value to the key as gas, so pot, key and gas are one signature; `placePickFor(player, …)` draws the stake from and refunds to `player` under the allowance the entry took, `PickFilled.player` is the player; the key can do nothing else and a finished match refuses it before the grant is consulted. Web: `useGameSession` (key in IndexedDB under `masayume.gameKey.`, `game-session` authority), picks routed through it, the receipt fed into the reducer at once, Flicky's auto-swipe at a card's cutoff on the side the book prices above even money. Deployed to Shannon at `0x0d8FC9659d02070aD8fF7E9a27E5394B9F5a2EF2` (block 479464205) after 7 new forge tests and the fork duel | Flicky `apps/web/src/routes/game/pvp.tsx` (no signature to reach the lobby), `active-duel.tsx` L504–549 (auto-swipe), its sponsored transactions throughout | A player signs the entry and then swipes; a card left to its last fifteen seconds is played for them on the favoured side and says so. The room's one message signature remains (per twelve-hour session, reload-safe) | **User decision 2026-09-04** ("One grant, then zero popups"); Adapted — Flicky sponsors, we grant; gas is the player's until a `SPONSOR_PRIVATE_KEY` exists (Blocked, recorded) |
| 2026-09-04 | **The parlay's four refusals made honest** — `maxStakeBase` carries the Range lane's 300 bps headroom, so a quote that drifts a fraction lands instead of bouncing into "changed from 5 to 5"; the requote sentence is the headline, not a line behind "technical details"; a leg follows its lane when its Window rolls (the reference re-syncs a leg's expiry from the refreshed oracle list — a Window here is a new id per round, so the leg moves to the lane's soonest live Window); `ThinBook(filled, depth)` is named on the leg's row and in the quote line, never "try a smaller stake" (the floor is fixed); the slip polls at 8 s | `ParlayBuilder.tsx` L145–159 (the roll), L225–245 (submit, no requote guard: opener-supplied probabilities) | A parlay places at the price shown or says the new price in one sentence; a leg no longer dies when the 1-minute Window rolls; a thin leg says which leg and how many contracts rest | User 2026-09-04 ("changed from 5 to 5", "odds should update by themselves", "not enough liquidity") |
| 2026-09-04 | **The reel bounded the reference's way** — one round per asset and lane, soonest bell first, capped at twelve (the reference: one per cadence, three); the card memoised with the clock read inside its head; every live read gated on `near` (the lane set already carries each opening print); the take cards on a minute's grain; `?m=` written with `replaceState` and restored on return; ↑/↓ between cards (additive, neither reference had it) | `app/reels/page.tsx` L268–273 (`rounds`), L252–282 (the observer gating the redraw), L9–10 and L172–190 (`<Link href="/markets">`, kept) | The reel no longer grows to a hundred subscribed cards; coming back from `/markets` lands on the card you left; the keyboard moves the feed | User 2026-09-04 ("reels lag"; "keep Yosuku's behaviour" for the links) |
| 2026-09-04 | **Asset marks are the reference's own drawings** — `BitcoinIcon.tsx`'s disc and ₿ path, `ChainIcon.tsx` L68–84's Ethereum diamond — on all four discs (hero badge, rail card, reel, word board); the typed "₿"/initial stays only for an asset neither drew | `components/icons/BitcoinIcon.tsx`; `components/ChainIcon.tsx` L68–84; discs at `app/markets/page.tsx` L79, L302, L699, `app/reels/page.tsx` L130, `WordMarketBoard.tsx` L104 | ETH shows the Ethereum diamond instead of an "E" on a grey disc; BTC the real mark instead of a typed glyph | **User decision 2026-09-04** ("use the real ETH logo", said three times) — the user's choice over the reference's typed glyph, in the reference's own drawing |
| 2026-09-04 | **The Ticket's first block, the drawer's live chart** — `lg:hidden`, a 128px chart in a rounded box, the same `PriceChart` as the hero's — was listed in the nine-block rebuild and not built; it is now | `Ticket624Drawer.tsx` L851–855 | On a phone the drawer opens with the Window's chart above the mode switch, as the reference's does | Exact; a slice-1 omission closed |
| 2026-09-04 | **The hero foot's alerts trigger takes the foot's grammar** — the Room's 9px mono uppercase text, not the bordered pill from the reference's Share/Copy/Alert row (a row the foot never had) | `app/markets/page.tsx` L758–777 (`.hero-chart-foot`: the Room button and the ramp, nothing else); `PriceAlerts.tsx` L50–56 | Room · Alerts · ramp on one 10px line at every width; armed reads vermilion as text | Placement still **Approved (owner, 2026-09-04)** (row of 2026-09-02); the presentation now matches the strip it sits in |
| 2026-09-04 | **`/surface` §02 drawn in `drawIvLine`'s grammar** — the depth plot gets the reference's 44px label column, three bands of grid, x labels along the bottom and the figures in the mono line beneath the box, the same grid §04's curve already used; it had been a bare stretched SVG with no axis | `app/surface/page.tsx` L31–83 (`drawIvLine`), L279–292 (the §02 box and its parameter line) | Contracts read up the side and prices along the bottom; the best bid/ask and totals sit under the box, not inside it | User 2026-09-04 ("that depth section… a mess"); Adapted content on the reference's exact frame |
| 2026-09-04 | **The Room's gate reads a bettors registry first** — every confirmed fill on every lane (wallet, Trading Balance, 2×, private) is recorded by `/api/room/bet` after the server reads the receipt; the position read stays as the fallback. The semantics become the reference's "ever bet" | `lib/sui/comments.ts` L59–84 (`bet_registry::record` inside the bet tx); `useCommentRoom.ts` L83–90 | A 2×, private or Trading-Balance bet unlocks the Room the moment it confirms, and a redeemed Window keeps its Room | User-reported 2026-09-04 ("bettors only doesn't open after I bet"); restoration plan slice 4 |
| 2026-09-04 | **Tap-trading is a modal, on the reference's one dialog grammar.** Yosuku has no enable UI (its one-tap is sponsored and silent) and no bottom sheet anywhere — every overlay is a centred `#0d0d10` panel over a blurred scrim. Ours was a bottom `Sheet` painted `bg-popover` (`#404040`), inner cards darker than the panel, six inputs, a submit that scrolled off a phone. Now `SessionModalShell` (shared `styles/modal.css`), opening on the reference's `AccountSetup` moment — one sentence, one button — with the caps behind "adjust" at defaults and the receipt behind "what you are signing"; the footer never leaves the screen. The armed ticket's footnote says the tap needs no prompt | `AddFunds.tsx` L58–67; `AccountSetup.tsx` L63–97; the user 2026-09-04 ("a slider, badly organised, worse in dark") | A near-black centred modal with one line and one button; the caps a tap away; the same in light | Adapted (no reference source for a browser key) — user-directed |
...
| 2026-09-03 | **Band presets follow the asset's price.** The reference's ±$15/30/55 and its $5 centre grid are BTC dollars; carried to another asset by its price ratio to the level they were tuned at (`PRESET_ANCHOR_USD` 77,000), the grid snapped to the nearest nice step, edges printed in cents under $10k. Found in a browser: on ETH every band was refused as a near-certainty (ETH 4h Balanced spanned $414, thirteen σ) | `ticket624.core.ts` L222–230; `Ticket624Drawer.tsx` L64–66; context/49 §1 | BTC keeps the reference's numbers to the dollar; ETH 5m Balanced ±$1.00 prices at 3.2× · 28.0% inside — the same odds band as BTC's | **Approved (owner, 2026-09-04)** — an adaptation where the reference had one asset |
...
| 2026-09-03 | **The LP Provider badge reads the maker vault's shares** — the reference's own `plpBalance > 0` (`lib/badges.ts` L48–52); it stays locked with its dependency named only where no vault is deployed. "Needs Earn (Stage 5)" was false once the vault went live | `lib/badges.ts`; context/49 §9 | A supplier's portfolio shows the badge unlocked; withdrawing every share locks it again (the vault keeps no per-wallet supply history) | **Approved (owner, 2026-09-04)** — the same narrowing as the Room's gate |
...
| 2026-09-02 | **A band's odds are the house's, from the Window's book and the venue's own prints.** `RangeReserve` centres a normal return where the Window's resting book puts the market (Φ⁻¹ of its P(close ≥ open) over the parlay's depth) and widens it by a per-asset σ measured from the venue's closing prints; a band opened mid-Window is therefore priced against the current price. The reference's venue priced bands with its own model; DreamDEX has none | `ticket624.core.ts` §range; `RangeMath.sol`, `RangePricing.sol`; context/43 §What this settles | The multiple on the button follows the market and the clock, and the chain charges exactly it or requotes | **Approved (owner, 2026-09-04)** — a pricing model where the reference had a venue; parameters are public and tunable |
...
| 2026-09-02 | `/games/range` is built on the parlay page's frame because the reference has no range page; the Ticket's range mode is the reference's own | ledger §RangeReserve | A game page in the app's grammar | **Approved (owner, 2026-09-04)** |
| 2026-09-02 | The range band is not yet drawn on the hero's chart (the reference shades it); recorded as a follow-up | `Ticket624Drawer.tsx` L750–761 | The band lives in the Ticket only | **Approved (owner, 2026-09-04)** |
...
| 2026-09-02 | The vault has no admin, no pause and no owner; the reference's admin-only `write_off_locked_for` has no counterpart | `trading_vault.move` L36, L336; doc 02 "pause does not trap owner exits" | Nothing can freeze a withdrawal; a lost position simply settles to what the chain pays | **Approved (owner, 2026-09-04)** — a deliberate removal |
...
| 2026-09-02 | Reputation tiers ported without the reference's per-tier bonus % and fee % | `lib/predictionContract.ts` TIERS | Tier, record and progress shown; no bonus or fee promised | **Approved (owner, 2026-09-04)** — no contract pays either |
| 2026-09-02 | Reputation, badges, the equity curve and CSV export are mounted on `/portfolio` §03 "Your record" | The reference computes all four on its portfolio page (`computeBadges`, `fetchReputation`, `equityRef`, `positionsToCSV`) but its pinned JSX mounts none of them; `BadgeDisplay` has no render site | A surface the reference wrote but never showed | **Approved (owner, 2026-09-04)** — placement is ours, components are the reference's |
...
| 2026-09-02 | Public pages rewritten on real facts only: `/docs` (contracts from the pinned addresses, code from the repo), `/how-it-works` (the reference's 64/36 example kept and labelled an example; fees truth-corrected), `/pitch` (labels CONCEPT / NEXT / NO-OP / ILLUSTRATIVE; team = the one builder in `git log`; no revenue rate), `/demo` (live traction from `/api/leaderboard`, real screenshots, real tx links) | doc 03 §Public/support surfaces; doc 05 §No fake-data | No future behaviour as current; every number sourced | **Approved (owner, 2026-09-04)** — `/docs` links `github.com/Blockchain-Oracle/masayume` (drop if private); the pitch's Somnia mark is a drawn stand-in |
...
|---|---|---|---|
| Trading balance block — `app/portfolio/page.tsx` at `1a36ffa^` L416–466: the pinned source computes the vault balances (L50–130) but its JSX mounts no controls; the block was cut in `1a36ffa` "drop the old-deployment section" | `TradingBalanceView`, `VaultControls`, `vault.css` | Restored from the reference's own history, as `/news` was | **Approved (owner, 2026-09-04)** |
...
| — | `VaultGrants`: the live grants and revoke, under the cells | Additive — ours | **Approved (owner, 2026-09-04)** — may overlap the session manager and strategy surfaces |
...
| Reference | Ours | Class | Approval |
|---|---|---|---|
| Linking: `api/claim/x/link` (sign in with X, then a wallet signature over `xLinkMessage`); `trade-from-x`'s tweet-a-code flow calls an external connect worker whose source is not in the repo | The `api/claim` flow, ported; the tweet-a-code step says so | Adapted | **Approved (owner, 2026-09-04)** |
...
| `/trade-from-x` L1–60, L139–156: "3x" leverage caps, two Suiscan proof links | The grant's real caps (per trade, per day, 8 open Windows, 30 days); the proof links point at `/docs` until real transactions exist; "settles back to you" kept because it is true | Truth correction | **Approved (owner, 2026-09-04)** |
| `/claim`: sealed auto-accounts funded by strangers' tweets | What waits is the Trading Balance of the wallet the account routes to; the reveal is a public read of that vault; the claim is connecting that wallet, with a signed re-link otherwise | Adapted | **Approved (owner, 2026-09-04)** |
| Tweet-vault cash-out to the wallet | The X betting balance is the EXECUTOR grant's budget; "Cash out" is `revoke`, returning the budget to the Trading Balance (copy says so) | Adapted | **Approved (owner, 2026-09-04)** |
| `/trade-from-x` dark island | Kept dark (`data-theme="dark"`) beside Reels per doc 04; the sticky strip keeps only the brand and "open the app" | Adapted | **Approved (owner, 2026-09-04)** |
...
| Reference | Ours | Class | Approval |
|---|---|---|---|
| Join in ONE signature (`LiveDesk.tsx`, one PTB) | Two signatures: vault `depositAndGrant`, then registry `subscribe` (which verifies the grant on the vault and pulls the fee to the creator; the registry never holds funds); the copy says so | Truth correction | **Approved (owner, 2026-09-04)** |
| Creator studio wears "Coming soon" and never lists (`app/strategies/page.tsx` L720–733) | Same markup, live: publish a spec the house runner honours | Adapted | **Approved (owner, 2026-09-04)** |
...
| Withdraw the desk balance without pausing (L376–390) | "Desk balance" is the grant's budget; withdrawing revokes first, so it pauses copying | Adapted | **Approved (owner, 2026-09-04)** |
...
| Leverage risk tiers | Caps: guarded = 1 open, price ≤ 0.70; balanced = 1 open, ≤ 0.85; active = 2 open, any price | Adapted — DreamDEX has no leverage | **Approved (owner, 2026-09-04)** |
...
| Reference | Ours | Class | Approval |
|---|---|---|---|
| Presets ±$15/30/55, scaled 0.5 / 1 / 4 for 1m / 5m / 1h; a $5 centre grid; whole dollars | the same three on BTC to the dollar; other cadences scale by √time from the 5-minute anchor (15m ≈ 1.7, 4h ≈ 6.9, 1d 16); other assets scale by price ratio to the level the dollars were tuned at, with the grid snapped to a nice step and edges in cents under $10k (ETH: ±$0.40/$1.00/$1.80 on $0.20 at 5m) | Adapted — lanes and assets the reference did not have; the price scaling is the decision-log row of 2026-09-03 | **Approved (owner, 2026-09-04)** |
...
| The band drawn as a shaded region on the price chart (`drawPriceLine` `band`) | **not drawn** — the chart card and the Ticket are siblings; lifting the band into the hero is a follow-up | Pending | **Approved (owner, 2026-09-04)** |
...
| — | `/games/range` with OUTSIDE as a side, the Window picker, the slip's Settle / Void (no oracle answer) / Claim pills | Additive — the game the doc-04 mode row names | **Approved (owner, 2026-09-04)** |
...
| "Withdraw all" | "Withdraw N idle" when capital is deployed; every closed unsettled Window is settled first (the page sends the permissionless crank, one signature each, asking the vault again after each — four were closed at once when the flow ran, context/49) | Adapted | **Approved (owner, 2026-09-04)** |
| — | §02 the Windows table with Merge / Settle; its labels read in one round (`useMarketsLite`), "…" only until they land | Additive | **Approved (owner, 2026-09-04)** |
...
| Funding source | the Trading Balance with a wallet top-up in the same PTB | the wallet only (the vault has no outbound path, AD-5); the chips lock off the wallet route | Deviation | **Approved (owner, 2026-09-04)** |
...
| One sentence under the strip: "L× can knock out before expiry." | The sentence, then one line on how a knock-out settles; the front, the fee and the line sit in the card's header and bar legend | Additive (truth) | **Approved (owner, 2026-09-04)** |
| Leverage column `1.0×` on every open bet (`Portfolio624Section` L452) | The multiple only on boosted rows; plain rows say nothing | Adapted — the recorded Stage 2 deviation, resolved | **Approved (owner, 2026-09-04)** |
| `LeveragePortfolioPanel`: value, cashout, live P&L, health % — defined at `app/portfolio/page.tsx` L368 and never mounted (`PORTFOLIO_UX_SPEC.md` L398 lists it among the deletions); the live reference UI is the `1.0×` column | "Yours now" (mark − front), the line or "at the knock-out line"; no percentage health | Adapted | **Approved (owner, 2026-09-04)** |
...
| — | The panel's trust and correlation sentences: a stolen desk key could redirect what an owner allowed it, so the allowance is the blast radius (keep it to a few bets); the charge and the slot's funding stay visible seconds apart | Additive (truth) | **Approved (owner, 2026-09-04)** |
...
| Position gate | on-chain `market_room_rule::join` → `bet_registry::has_bet` | `gate.server.ts` `holdsPosition`: `hasBet` first, `listOpenPositions` as the fallback | Adapted | **Done** — since 2026-09-04 a `bettors` registry (`packages/db/src/bettors.ts`, written by `/api/room/bet` once the receipt is verified against the caller) carries the reference's "ever bet" on every lane, including the three whose tokens never reach the wallet (2×, private, Trading Balance); the position read remains for wallets that bet before the table existed |
...
| Mount | old market page's Share/Copy/Alert row, removed in 8c7ecc1 | hero foot, beside the Room | **Placement ours** | **Approved (owner, 2026-09-04)** |
...
| `/strategies` | `app/strategies/page.tsx` | Adapted | `StrategyRegistry` + DB | **Partial** — the desk is live (Stage 4); since 2026-09-04 (`1e3f189`) a creator can launch an **AI agent** preset (persona, posture, cadences; one model read per Window gated deterministically; a dry read from the studio; memory on the card). The runner needs `AI_MODEL` + a key on Fly before an agent trades |
...
| `/trade-from-x` | `app/trade-from-x/page.tsx` | Adapted | X provider + `EventVault` grant | **Done** — proven live 2026-09-05: Sign in with X (OAuth 1.0a), link, fund + authorize, then `@masayume_app eth up 1 1h go` became a filled 1 tUSDC order (`0x4bcbc318…c484`) with the receipt on the page and the reply on X (context/58 §7) |
...
| `/games/lucky` | Randomized live-market mode, auditable selection | Additive | **Partial** — built 2026-09-04 (`e863a42`): committed server seed, client seed, HMAC draw, eligibility scan, the deal card with its proof, one order through the Ticket lane, verdicts/history/streak; the live spin waits on a venue with open 5m/15m Windows (context/58 §6) |
| `/games/range` | Range market backed by `RangeReserve` | Additive | **Shell** — honest dependency state |
| `/games/moonshot` | PIPS-derived; economic nature stated plainly | Additive | **Partial** — built 2026-09-04 (`fb426e1`, option A): the strike solved in core and pinned to `RangeMath` by golden vectors, the reserve's own `previewOpen` agreeing live (ETH 4h 5× → 5.005×, context/58 §5); a live round pending |
| `/games/line-rider` | PIPS-derived arcade | Additive | **Done** offline 2026-09-04 (`2b89fd6`): seeded fixed-step engine, canvas stage, a score the server replays before it believes it; a posted score against the database pending |
| `/games/candle-hop` | PIPS-derived arcade | Additive | **Done** offline 2026-09-04 (`2b89fd6`): as Line Rider |
