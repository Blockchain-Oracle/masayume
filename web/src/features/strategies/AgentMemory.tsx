...
function MemoryRow({ d, asset, nowMs }: { d: DecisionWire; asset: string; nowMs: number }) {
  ...
  return (
    <div className="strat-memory-row">
      <div className="strat-mono-10 flex items-baseline justify-between gap-2 text-white/40">
        <span className="truncate">
          {ago(d.decidedAtMs, nowMs)}
          {d.intervalSec !== null && ` · ${asset} ${formatCadence(d.intervalSec)}`}
        </span>
        {d.outcome && <span className={cn("shrink-0 uppercase tracking-[0.12em]", d.outcome === "won" ? "text-vermilion" : d.outcome === "lost" ? "text-white/60" : "text-white/30")}>{M.outcome[d.outcome]}</span>}
      </div>
      <div className="strat-mono-11 mt-1 flex flex-wrap items-baseline gap-x-2">
        <span className={cn(d.gate === "failed" ? "text-white/50" : "text-white")}>{call}</span>
        <span className={cn(d.gate === "trade" ? "text-vermilion" : "text-white/50")}>→ {ruling}</span>
      </div>
      <p className="mt-1 text-xs leading-snug text-gray-300">“{d.why}”</p>
      {d.gate !== "trade" && <p className="strat-mono-10 mt-0.5 text-white/35">{d.gateReason}</p>}
    </div>
  );
}
...
export function AgentMemory({ agent, storeConnected, asset, nowMs }: AgentMemoryProps) {
  return (
    <div className="mb-4 border border-vermilion/30 px-4 py-3">
      ...
      <p className="strat-mono-10 mt-1.5 truncate text-white/40">{agent.model ? H.model(agent.model) : H.noModel}</p>
      {agent.decisions.length === 0 ? (
        <p className="strat-mono-11 mt-2 text-gray-500">{storeConnected ? M.empty : M.storeOff}</p>
      ) : (
        <div className="mt-2">
