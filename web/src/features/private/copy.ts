...
export const PRIVATE = {
  ...
  cta: {
    ...
    notPlaced: "Private bet not placed",
  },
  quote: {
    cost: "Cost",
  ...
  toasts: {
    ...
    unknown: "The chain has not answered yet — tap again to resume; nothing is charged twice.",
    flippedOff: (reason: string) => `Private mode went away (${reason}); this bet would come from your wallet. Choose Private again once it is back.`,
    resumed: (window: string) => `Resumed: the private bet on ${window} was already placed.`,
  ...
  claims: {
    ...
    backUp: "Back up",
    restore: "Restore",
    restored: (n: number, skipped: number) => `Restored ${n} claim${n === 1 ? "" : "s"}.${skipped > 0 ? ` ${skipped} row${skipped === 1 ? "" : "s"} in the file could not be read and ${skipped === 1 ? "was" : "were"} skipped.` : ""}`,
    skipped: (n: number) => `${n} row${n === 1 ? "" : "s"} in that file could not be read; nothing was restored.`,
    home: (amount: string, symbol: string) => `${amount} ${symbol} came home`,
